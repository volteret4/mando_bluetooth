...
class BTHIDKeyboard:
    ...
    def _verify_sdp(self) -> None:
        ...
        if "Human Interface Device" in r.stdout:
            log.info("SDP record verified: HID service present")
        else:
            log.warning("SDP HID record not found after registration — TV may not see HID service")

    def _register_sdp_dbus(self) -> None:
        """Register HID SDP record via BlueZ ProfileManager1 (fallback)."""
        try:
            import dbus
...
<record>
  ...
  <attribute id="0x0210"><uint16 value="0x0320"/></attribute>
            ...
            manager.RegisterProfile(
                "/org/mando_bluetooth/hid_sdp",
                "00001124-0000-1000-8000-00805f9b34fb",
                opts,
            )
            log.info("HID profile registered via DBus ProfileManager1")
        except Exception as e:
            log.error("DBus SDP registration failed: %s", e)
            log.error("No SDP record active — pair the TV and then connect manually if needed")
